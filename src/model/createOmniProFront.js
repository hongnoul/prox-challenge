...
function roundedBoxGeometry(width, height, depth, radius, bevel = 0.06) {
...
function scallopedGeometry(radius, depth, scallops = 10, recess = 0.12) {
  const shape = new THREE.Shape();
...
function createCanvasLabel(text, foreground, background, fontWeight = 700) {
 ...
 * - product.webp: orange/graphite enclosure, rounded protective fascia,
 ...
 * Approximate envelope is 12 W x 17 H x 21 D, matching the existing viewer.
 ...
 * @param {object} [options.palette] Numeric Three.js colors keyed like DEFAULT_PALETTE.
 * @returns {THREE.Group} The model root. Find controls with getObjectByName(),
 * and call root.userData.dispose() when permanently removing it.
 */
export function createOmniProFront(options = {}) {
  const {
    castShadow = true,
    receiveShadow = true,
  ...
  const material = (color, roughness = 0.68, metalness = 0.08, extra = {}) => {
  ...
  root.userData.nominalEnvelopeInches = Object.freeze({ width: 12, height: 17, depth: 21 });

  const addMesh = (name, geometry, meshMaterial, parent, position = [0, 0, 0], rotation = [0, 0, 0]) => {
  ...
  const addBox = (name, size, position, meshMaterial, parent, rotation = [0, 0, 0]) => addMesh(
    name,
  ...
  const addRoundedBox = (name, size, radius, position, meshMaterial, parent, bevel = 0.06) => addMesh(
    name,
  ...
  const addTube = (name, points, radius, meshMaterial, parent, closed = false) => {
  ...
  const addLabel = (name, text, size, position, parent, labelOptions = {}) => {
    const {
      foreground = '#f2f0e8',
      background = null,
      fontWeight = 700,
  ...
  const createSemanticGroup = (name, parent, position) => {
  ...
  const createPushButton = (semanticName, x, labelText) => {
  ...
  const createKnob = (semanticName, x, y, radius, pointerAngle = 0) => {
  ...
  const createDinseSocket = (semanticName, x, polarity) => {
  ...
  root.userData.dispose = () => {
    ...
    root.traverse((object) => {
      if (object.isMesh && object.geometry && !disposedGeometries.has(object.geometry)) {
